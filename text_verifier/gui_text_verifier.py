...
class TextVerifier:
    ...
    async def process_text(self, file_path, update_queue):
        ...
        try:
            ...
            if COLUMN_BEFORE not in df.columns:
                update_queue.put((f'엑셀 파일에 {COLUMN_BEFORE} 열이 없습니다.', 0))
                return
            ...
            async with aiohttp.ClientSession() as session:
                for idx, row in df.iterrows():
                    ...
                    try:
                        ...
                        df.at[idx, COLUMN_AFTER] = response["message"] if response["success"] else "실패"
                    except Exception as e:
                        df.at[idx, COLUMN_AFTER] = f"오류: {str(e)}"
